...
/****camera_feed.cpp********************************************************************
1. Decompresses jpeg data to a specified feed
    1.a The parameters are the decompressor, the compressed image, the size of the
...
2. Initializes to turboJPEG decompressor
    2.a If decompressor fails to construct, send an error
3. Initializes a feed with given name, width and height
    3.a Allocates space for frame data within each frame buffer.
    3.b Initialize with an indicator of no feed.
4. Handles sections of video that come from rover to ensure the feed is in the correct order
    4.a Since we know that camera packets will only be sent out once we
    ...
    4.b If we have done a full loop and there is a new
        frame index, then we overwrite the old data.
    4.c This section handles any data that comes out of order.
        Each data section is written at the size of
        a signle data packet times the section index.
    4.d This frame is ready. Decompress and update the texture.
        4.d.i If an error occured on the decompress, return the error
5. Deletes the inputted feed
    5.a Deletes data sections contained in the feed
****************************************************************************************/

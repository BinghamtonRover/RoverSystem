...
int main() {
	...
	camera::Error err1 = camera::open(&session1, "/dev/video1", 1366, 768);
	err1= camera::start(&session1);
	std::cout << camera::get_error_string(err1) << std::endl;

	camera::Error err2 = camera::open(&session2, "/dev/video2", 1366, 768);
	err2= camera::start(&session2);
	std::cout << camera::get_error_string(err2) << std::endl;

	camera::Error err3 = camera::open(&session3, "/dev/video3", 1366, 768);
	err3= camera::start(&session3);
	std::cout << camera::get_error_string(err3) << std::endl;

	camera::Error err4 = camera::open(&session4, "/dev/video4", 1366, 768);

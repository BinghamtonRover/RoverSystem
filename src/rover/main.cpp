...
Config load_config(const char* filename) {
    ...
    if (err != sc::Error::OK) {
        logger::log(logger::ERROR,"Failed to parse config file: %s\n", sc::get_error_string(sc_config, err));
        exit(1);
    }
    ...
    if (!suspension_serial_id) {
        logger::log(logger::ERROR,"Config file missing 'suspension_serial_id'!\n");
        exit(1);
    }
    ...
    if (!imu_serial_id) {
        logger::log(logger::ERROR,"Config file missing 'imu_serial_id'!\n");
        exit(1);
    }
...
int main()
{
	...
	if (suspension::init(config.suspension_serial_id) != suspension::Error::OK) {
		logger::log(logger::ERROR, "[!] Failed to initialize the suspension!\n");
		return 1;
	}

	if (lidar::start("192.168.1.21") != lidar::Error::OK) {
		logger::log(logger::ERROR, "[!] Failed to init field LIDAR!\n");
		return 1;
	}

#if 0
	if (imu::start(config.imu_serial_id) != imu::Error::OK) {
		logger::log(logger::ERROR, "[!] Failed to start IMU!\n");
		return 1;
	}
#endif
    ...
    for (int i = 0; i < MAX_STREAMS; i++) {
		...
		logger::log(logger::INFO, "> Found camera with name %s\n", name_filename_buffer);
   ...
   logger::log(logger::INFO,"> Using %d cameras.\n",streams.size());
...

    {
        ...
        if (err != network::Error::OK) {
            logger::log(logger::ERROR,"[!] Failed to start rover feed: %s\n", network::get_error_string(err));
            exit(1);
        }
    }

    {
        ...
        if (err != network::Error::OK) {
           logger::log(logger::ERROR,"[!] Failed to subscribe to base station feed: %s\n", network::get_error_string(err));
            logger::log(logger::ERROR,"errno %d\n", errno);
            exit(1);
        }
    }
    ...
    while (true) {
        for (size_t i = 0; i < streams.size(); i++) {
            ...
            size_t frame_size;
            {
                ...
                if (err != camera::Error::OK) {
                    logger::log(logger::WARNING,"Camera 0:%s\n",camera::get_error_string(err));
                }
            }
		...
		if (lidar_send_timer.ready()) {
			...
			if (lidar::scan(lidar_points) != lidar::Error::OK) {
				logger::log(logger::WARNING, "[!] Failed to read LIDAR points!\n");
			}
...
*/
			if (tj_err != 0) {
				logger::log(logger::WARNING, "[!] tjCompress failed: %s\n", tjGetErrorStr2(compressor));
			}
        ...
        while (true) {
			...
			if (neterr != network::Error::OK) {
                ...
                logger::log(logger::WARNING,"[!] Network error on receive!\n");

...
Camera packets do not use the network protocol defined [in the network spec](network.md). Each camera packet is sent as a single UDP packet.

## Packet Header

Each camera packet has a common header:
...
4. Assign the next available frame index to all packets carrying `F`. The packets carrying `F` must all have the same
    frame index value.
5. Send the packets.

Here is an example. Say the latest frame from the camera, when encoded as a JPEG image, has a size of 141330 bytes.
Assume a `CAMERA_PACKET_FRAME_DATA_MAX_SIZE` of 40000 bytes. Then 4 packets will be created. Assume that the next frame index is 1234. Then the packets for this frame should appear as the following:
...
A camera packet is received. It has frame index of 21. Since neither buffer has a frame index of 21, this is a new frame. The packet has a section index of 0 and a section count of 4. The current buffer (BUFFER 0) is updated and the frame data is copied. The next buffer is set to 1.
...
Another camera packet is received. It has frame index 21, which is assigned to BUFFER 0. Thus this is not a new frame. The packet has a section index of 2 and a section count of 4. Notice this means that section 1 and section 2 are arriving out-of-order, but that is okay. BUFFER 0 is updated.
...
Another camera packet is received. It has frame index 21, which is assigned to BUFFER 0. The packet has a section index of 1 and a section count of 4. Buffer 0 is updated.
...
Another camera packet is received. It has frame index 26, which is not assigned to a buffer. Thus the next buffer is updated to 0, and BUFFER 1 is updated accordingly. The packet has a section index of 0 and a section count of 3.
...
Another camera packet is received. It has frame index 21, which is assigned to BUFFER 0. The packet has a section index of 3 and a section count of 4. Now BUFFER 0 is full, since its section count equals the packet's section count. BUFFER 0 is pushed to screen.
...
Another camera packet is received. It has frame index 28, which is not assigned to a buffer. Since the next buffer is 0 and BUFFER 0 has a non-zero frame index (meaning that it has at least a partial frame), we check its remaining count. Since that is 0, the buffer is full and thus not dropped. BUFFER 0 is then reset with the information from the new packet, and the next buffer is updated to 1. The packet has a section index of 1 and a section count of 2.
...
Yet another camera packet is received. It has frame index 29, which is not assigned to a buffer. Since the next buffer is 1 and BUFFER 1 has a non-zero frame index, we check its remaining count. Since that is non-zero, we have not received all the packets of frame 26, so the frame is dropped. Thus the next buffer is BUFFER 0 and BUFFER 1 is updated. The new packet has a section index of 0 and a section count of 2.
...
Of course, the next camera packet to arrive has frame index of 28, which we just discarded. Since the next buffer is 1, BUFFER 1 is discarded and filled with the new information. The new packet has a section index of 0 and a section count of 2.
...
The next camera packet has a frame index of 29, a section index of 1, and a section count of 2. BUFFER 1 contains this packet, so BUFFER 1 is filled. The next buffer is updated to 0. BUFFER 1 is then pushed to screen.
...
The next camera packet has a frame index of 30, a section index of 0, and a section count of 3. BUFFER 0 is the next buffer, so it is discarded (since it has a non-zero remaining count) and the next packet's information is filled.
